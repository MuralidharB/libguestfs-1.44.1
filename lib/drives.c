 ...
 */
static int
valid_secobject (char *secobject)
{
  /* Returns first token */
  char *id;
  char *data;
  char *key;
  char *token = strtok(secobject, ",");
			     ...
			     const struct guestfs_add_drive_opts_argv *optargs)
{
  ...
  data.secobject = optargs->bitmask & GUESTFS_ADD_DRIVE_OPTS_OBJECT_BITMASK
  ...
  if (data.secobject && !valid_secobject (data.secobject)) {
